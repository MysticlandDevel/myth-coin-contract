...
// const MythCoinMultiSigFactory = artifacts.require('MythCoinMultiSigWalletWithMint.sol');
...
//
// contract("MythCoinMultiSigWalletWithMint", (accounts) => {
...
//     assertNotRevert("signer1",() => walletInstance.requestMintInPlay(user1, 1, 0, "test addsigner-1", {from: user1}));
...
//     assertRevert("signer1",() => walletInstance.requestMintInPlay(user1, 1, 0, "test addsigner-1", {from: user1}));
...
//       assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OneMythCoinInMinunit, {from: user1}));
//       let afterBalance = await coinInstance.balanceOf(user1);
//
//       // balance difference should be same as the minted amount
//       assert.equal(afterBalance.sub(prevBalance).toString(), OneMythCoinInMinunit.toString());
//
...
//       assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OneMythCoinInMinunit, {from: user1}));
...
//         const thousandPlay = BIG(1000)
//         const zero = BIG(0)
//
//         const MSG1 = "MINT 1000 Play to user1";
//         const trSubmit = await walletInstance.requestMintInPlay(user1, thousandPlay, zero, MSG1, {from:vault});
//
//         //printTx(trSubmit);
...
//         assert.equal(afterBalance.toString(), thousandPlayInMinunit.toString());
//
