// 'use strict';
//
// const MythCoin = artifacts.require('MythCoin.sol');
// const MythCoinMultiSigFactory = artifacts.require('MythCoinMultiSigWallet.sol');
// const web3 = MythCoin.web3;
// const eth = web3.eth;
...
//   return MythCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
// };
//
// const deployMythCoin = (owner,admin,vault) => {
//   return MythCoin.new("Play", "MythCoin", owner, admin, vault);
// };
//
// const printTx = (tx) => {
...
//
// contract("MythCoinMultiSigWallet", (accounts) => {
//   const BIG = (v) => new theBN.BN(v)
//   let coinInstance;
//   let walletInstance;
//   const requiredConfirmations = 2;
//
//   let OneMythCoinInMinunit;
...
//
//   beforeEach(async () => {
//     coinInstance = await deployMythCoin(owner, admin, vault);
//     assert.ok(coinInstance);
//
//     NoOfTokensInMinunit = await coinInstance.totalSupply();
//     OneMythCoinInMinunit = await coinInstance.getOneMythCoin();
//     NoOfTokens = NoOfTokensInMinunit.div(OneMythCoinInMinunit);
//
...
//     assert.equal( await walletInstance.MythCoin(), coinInstance.address );
//
...
//     // user1 cannot
//     assertRevert("1-1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test 1", {from: user1}));
...
//
//     // owner cannot
//     assertRevert("2-1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test 2-1", {from: owner}));
...
//
//     // admin(as signer) can
//     assertNotRevert("3-1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test 3-1", {from: admin}));
...
//
//     // vault(as signer) can
//     assertNotRevert("4-1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test 4-1", {from: vault}));
...
//     assertNotRevert("5-1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test 5-1", {from: signer}));
...
//     // request a TX
//     walletInstance.requestTransferInPlay(user1, 1, 0, "test 1", {from: signer});
//
...
//
//     const thousandPlayInMinunit = OneMythCoinInMinunit.mul(BIG(1000))
//     coinInstance.transfer(walletInstance.address, thousandPlayInMinunit, {from:vault});
//     assert.equal(await balanceOf(walletInstance.address), thousandPlayInMinunit.toString());
//
//     // Add MythCoin transfer transaction
//     // example ABI generation:
//     // const transferData = coinInstance.contract.transfer.getData(user1, OneMythCoin * 1);
//     //
//
//     const MSG1 = "send 1Play and 1DENNIS to user1";
//     const trSubmit = await walletInstance.requestTransferInPlay(user1, 1, 1, MSG1, {from:vault});
//
//     //printTx(trSubmit);
...
//     assert.equal(afterBalance.toString(), OneMythCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
//
...
//     assertNotRevert("signer1",() => walletInstance.requestTransferInPlay(user2, 1, 0, "test addsigner-1", {from: user1}));
...
//     assertRevert("signer1",() => walletInstance.requestTransferInPlay(user1, 1, 0, "test addsigner-1", {from: user1}));
