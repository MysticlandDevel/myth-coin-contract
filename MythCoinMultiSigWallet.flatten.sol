...
// File: contracts/MythCoinSafeMath.sol
...
// File: contracts/MythCoinMultiSigWallet.sol
...
/**
 * @title MythCoin Multi Signature contract
 */
contract MythCoinMultiSigWallet {
    /*  Enums  */
    ...
    /* Contract Created */
    event MultiSigCreated(address indexed creator, address indexed MythCoin);
    ...
    using MythCoinSafeMath for uint8;
    using SafeMath for uint256;
     ...
     * @param _MythCoinAddress MythCoin address
     *
     */
    constructor (address[] memory _signers, uint8 _requiredSign, address _MythCoinAddress, string memory _description) public {
        ...
        nextTx = 0;
        MythCoin = MythCoin(_MythCoinAddress);
        owner = msg.sender;
        description = _description;
        emit MultiSigCreated(owner, _MythCoinAddress);
    }
    ...
    /**
      ...
      * @param _amountUnit amount of MythCoin(Play part) to be transferred
      * @param _amountMinunit amount of MythCoin(Minunit part) to be transferred
      */
    ...
    // MythCoin.transfer will be fail with reason out of gas. Then the TX will remain
    ...
    function runTX(uint16 txid) internal senderIsSigner returns(bool) {
        ...
        if(request.txType == TxType.TRANSFER) {
            // transfer money
            MythCoin.transfer(request.who, request.amount);
            ...
            MythCoin.setAdmin(request.who);
        } else if(request.txType == TxType.OWNER_CHANGE) {
            MythCoin.setOwner(request.who);
        } else if(request.txType == TxType.VAULT_CHANGE) {
            MythCoin.setVault(request.who);
        } else {
            return false;
        }
        return true;
